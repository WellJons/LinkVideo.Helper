...
Текущий production-канал публикует полный проверенный `LinkVideo.Helper_Setup.exe`; поле `patches` в manifest остаётся пустым. Дифференциальный patch pipeline собирается и проверяется release-gate как резерв на будущее, но не публикуется, пока не будет иметь эквивалентную полному Setup защиту от сбоя/отключения питания в середине обновления.
