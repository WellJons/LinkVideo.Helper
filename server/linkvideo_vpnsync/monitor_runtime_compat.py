...
def install_monitor_runtime_compat() -> None:
    ...
    The status helper intentionally exposes no credentials. It reports both
    thread liveness and whether a RouterOS API socket is currently connected.
    """
    ...
    def status(self) -> dict[str, Any]:
        ...
        for target in self.targets:
            ...
            if listener is not None:
                lock = getattr(listener, "_client_lock", None)
                if lock is not None:
                    with lock:
