        ...
        rows = [
            {
                ...
                # 3.0.13 operator-facing archive format: external ports only.
                "NAT / Порты": "10001; 10002 [off]",
...
def main() -> int:
    ...
    # Snapshot-less fallback must remain usable after visible NAT values became
    # compact. With no saved internal target the conservative rule is TCP 1:1.
    ...
    assert fallback[0]["protocol"] == "tcp"
    assert fallback[0]["dst-port"] == "10001"
    assert fallback[0]["to-ports"] == "10001"
    assert fallback[1]["dst-port"] == "10002"
    assert fallback[1]["to-ports"] == "10002"
    assert fallback[1]["disabled"] == "yes"

    # Legacy verbose archive rows are still accepted and retain a non-1:1 target.
    verbose = restore._fallback_nat(
        {"NAT / Порты": "udp 12000→554"},
        "legacy",
        "172.16.1.20",
    )
    assert len(verbose) == 1
    assert verbose[0]["protocol"] == "udp"
    assert verbose[0]["dst-port"] == "12000"
    assert verbose[0]["to-ports"] == "554"
    ...
    compat_source = (
        ROOT / "linkvideo_vpn_helper" / "services" / "vpn_restore_compact_ports.py"
    ).read_text(encoding="utf-8")
    ui_source = (
    ...
    assert "_fallback_nat_compact" in compat_source
