...
def install_cloud_activity_bridge(service: VPNService, settings) -> None:
    """Mirror successful/failed Helper mutations into central VPNSync audit.

    This is a transition bridge while operational pages are moved from direct
    RouterOS calls to server-side write endpoints. The bridge is intentionally
    non-blocking and never changes the result of the underlying RouterOS action.
    """
    global _INSTALLED
    if _INSTALLED:
        return
    ...
    def submit(action: str, *, server: str, login: str, success: bool, details: dict | None = None) -> None:
        def worker():
            try:
                ...
                # Central RouterOS listener/PostgreSQL state remains authoritative;
                # an audit transport failure must not break the employee action.
    ...
    event("CLOUD", "Аудит действий Helper подключён", "VPNSync activity")
    _INSTALLED = True
